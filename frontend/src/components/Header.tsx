...
export default function Header({ totalVotes }: { totalVotes: number }) {
  ...
  return (
    <header className="border-b border-white/[0.06] bg-[#110e14]/80 backdrop-blur-md sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-5 py-3 flex items-center justify-between gap-2">
        ...
        <nav className="flex items-center gap-1">
          ...
          {navLink('/about', 'About')}
          {totalVotes > 0 && (
            <span className="hidden sm:inline text-xs text-[#5a5660] ml-3 tabular-nums">
